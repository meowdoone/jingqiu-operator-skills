...
先回答这次是商品无法卖、详情没讲清、需要视频，还是推广回收不好。只处理用户指定工作，不能把过期操作或西班牙/巴西本地模式套成所有 AE 账号的现行规则。
...
用真实商品与使用场景解释购买问题，不预设固定 AE 视频回报算法。当前账号的上传位置、审核、比例及 AI 标注要求需要现场核验；不称已接视频 API。镜头中的包装、配件、功能与声明逐项核真实交付。
...
关键词、商品发布、物流模板、本地库存和运费按当前市场逐项核验。中国跨境托管模式、当前竞价及视频增量效果须另用对应账号和实际结果验证，不默认已覆盖。
