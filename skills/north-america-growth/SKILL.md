...
读取 [专家依据与适用范围](../../research/expert-north-america-growth-2026-09-08.md)。按需要查看 [付费获客](../paid-acquisition/SKILL.md) 或 [GEO](../geo-distribution/SKILL.md)，不把两者合并成同一个结果指标。
...
给出：目标 → 事实 → 主要瓶颈 → 本周任务与负责人 → 成本边界 → 验收 → 下次检查时间。把专家观点、本次假设、已完成动作分别标明。对外文案用业务语言；保留必要字段，不堆叠 Agent 术语。
